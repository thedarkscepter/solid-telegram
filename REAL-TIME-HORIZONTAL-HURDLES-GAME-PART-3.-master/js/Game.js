...
class Game {
  ...
  play(){
    ...
    if(playerid === 2){
      var trysRef  = database.ref('players/player1/trys');
      trysRef.on("value",function(data){
      otrys = data.val();
      })
     
    }
    if(playerid === 1){
      var trysRef  = database.ref('players/player2/trys');
      trysRef.on("value",function(data){
      otrys = data.val();
      })
      console.log(otrys)
    ...
    if (playerid === 1 ){
      ...
      if(player.jump >= -30 && ( player.distance === 2100||player.distance === 3200) ){
        ...
        if (trys === "fail"){
        rankshow = createElement('h1');
        rankshow.position(displayWidth/2, displayHeight/2-30);
        rankshow.html("You lost :(");
        }

        if (trys === "pass"){
          rankshow = createElement('h1');
          rankshow.position(displayWidth/2, displayHeight/2-30);
          rankshow.html("You Won :)");
          }
        
  
        if (otrys === "fail" && trys === "fail"){
          rankshow = createElement('h1');
          rankshow.position(displayWidth/2, displayHeight/2-30);
          rankshow.html("tie");
          }
          if (otrys === "pass" && trys === "pass"){
            rankshow = createElement('h1');
            rankshow.position(displayWidth/2, displayHeight/2-30);
            rankshow.html("you came" + rank);
          }
      }
    }
    ...
    if(gameState === 3){
      ...
      if(playerid === 1){
        var trysRef  = database.ref('players/player0/trys');
        trysRef.on("value",function(data){
        otrys = data.val();
        })
        console.log(otrys)
